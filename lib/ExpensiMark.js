    ...
    replace(text, {filterRules = [], shouldEscapeText = true} = {}) {
        ...
        rules.forEach((rule) => {
            ...
            if (rule.process) {
                ...
                // The 'codeFence' rule returns a string where text is surrounded by
                // <pre> tags. We're leveraging this to confirm the rule is applied
                // before removing trailing/leading backticks. Removing the backticks
                // without this check would mean other rules that rely on backticks will
                // not be applied e.g. `test` would be displayed as just 'test' because the
                // backticks will be already stripped before the text reaches the 'inlineCodeBlock'
                // rule. This is important since 'codeFence' is the first rule.
                if (replacedText.match(/^(?:&#x60;)+<pre>.+<\/pre>(?:&#x60;)+/g)) {
                    // Remove leading and trailing backticks i.e. &#x60; if 'codeFence' rule is applied.
                    // Subsequent code block rules can't format text based on these backticks anymore which caused bugs
